...
Function level_editor()
	...
	Repeat
		...
		info_x = round_to_nearest( SETTINGS_REGISTER.ACTUAL_WINDOW_WIDTH.get(), gridsnap ) - 300; info_y = 0;
		SetAlpha( 0.75 )
		SetColor( 32, 32, 32 )
		DrawRect( info_x,info_y, 300,SETTINGS_REGISTER.ACTUAL_WINDOW_HEIGHT.get() )
		SetAlpha( 1 )
		SetColor( 196, 196, 196 )
		DrawLine( info_x,info_y, info_x,SETTINGS_REGISTER.ACTUAL_WINDOW_HEIGHT.get() )
		SetColor( 255, 255, 255 )
		info_x :+ 8; info_y :+ 10
				...
				Case EDIT_LEVEL_MODE_UNIT_FACTORY_DETAILS
					...
					If uf <> Null
						...
						For Local r% = 0 To uf.count_squads()-1
							'wave index
							SetColor( 255, 255, 255 )
							DrawText_with_shadow( uf.wave_index[r], SETTINGS_REGISTER.ACTUAL_WINDOW_WIDTH.get() - 68, info_y + r*cell_size + line_h/3 )
							'draw all delay times except the cursor
							If r <> cursor
								SetColor( 127, 127, 127 )
								DrawText_with_shadow( uf.delay_time[r], SETTINGS_REGISTER.ACTUAL_WINDOW_WIDTH.get() - 50, info_y + r*cell_size + line_h/3 )
							End If
						Next
	
						If KeyHit( KEY_ENTER ) And cursor >= 0 And cursor < uf.count_squads()
							FlushKeys()
							uf.delay_time[cursor] = get_input( uf.delay_time[cursor],, SETTINGS_REGISTER.ACTUAL_WINDOW_WIDTH.get() - 50, info_y + cursor*cell_size + line_h/3, input_font, screencap() ).ToInt()
						End If
						If cursor >= 0 And cursor < uf.count_squads()
							SetColor( 127, 127, 127 )
							DrawText_with_shadow( String.FromInt( uf.delay_time[cursor] ), SETTINGS_REGISTER.ACTUAL_WINDOW_WIDTH.get() - 50, info_y + cursor*cell_size + line_h/3 )
						End If
